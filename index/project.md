...
## Narzędzia przygotowania assetów Tycho

Pojedyncze konwersje assetów Tycho zapisuje się w `tools/` jako `prepare_tycho_*.py`, razem z istniejącymi `prepare_tycho_bench.py`, `prepare_tycho_book.py`, `prepare_tycho_seating.py`, `prepare_tycho_station.py` i `prepare_tycho_modules.py`. `prepare_tycho_habitat_tunnel.py` oraz `prepare_tycho_fish.py` stosują tę samą konwencję, ale nie należą do wspólnej listy `MODULES` w `prepare_tycho_modules.py`, ponieważ `modules.json` jej nie odzwierciedla.
...
Lokalny silnik jest w `tools/godot/Godot_v4.6.1-stable_win64_console.exe`. Testy `res://tests/*.gd` (rozszerzają `SceneTree`) odpala się przez `--headless --path godot --script tests/<nazwa>.gd`; testy sterowane samą grą (`--smoke-test`, `--climb-test`, `--road-test` i inne flagi w `moonwalk.gd`) potrzebują pełnej sceny: `--headless --path godot scenes/moonwalk.tscn -- --climb-test`. Render automatyczny wymagający okna uruchamiaj z argumentem użytkownika `--agent-run`, który pozostawia widoczny kursor zamiast przechwytywać go przez grę. `--smoke-test` potrafi trwać dłużej niż 90 s — nie ucinaj go krótkim timeoutem, inaczej brak wyniku wygląda jak cichy fail. Stały szum silnika przy `quit()` w headless (`ERROR: BUG: Unreferenced static string`, `Pages in use exist`, ostrzeżenie o Thread) nie jest błędem.
