...
- Theia pozostaje dostępna: `powershell -ExecutionPolicy Bypass -File .\START.ps1 -Theia` (lub argument gry `--theia`). `START.cmd` uruchamia Agnes. Kopie modeli Agnes przygotowuje `tools/prepare_agnes.py`; animacje używają jednej wspólnej tekstury. Oryginały w `artifacts/sprites/agnes` pozostają bez zmian. Emisję materiałów wyłączono, aby postać reagowała na światło i cienie.
