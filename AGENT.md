...
**Ciąg dalszy z tego samego dnia (2026-09-12), po naprawie fasady: gracz faktycznie odpalił grę i zgłosił dwa kolejne, realne problemy niewidoczne w testach jednostkowych.** (1) Cokoły (kolizja warstwy 1, blokująca ruch) wielu budynków w Tycho były pojedynczym `BoxShape3D` o wymiarach z `dimensions[kind].size_m` — czyli całym bounding boxem. Dla prostokątnych brył to trafne, ale `l-shape-building` w ten sposób ma solidną kolizję nawet we własnej brakującej ćwiartce, a `block-of-flats`/`central-building`/`central-house` to w rzeczywistości ośmiokąt / wydłużony wielokąt / rotunda ~50 m (nie bug skali importu, jak zakładał stary komentarz — promień jej śladu trzyma się 25,0-25,25 m przy 16 próbkowanych kątach) wpisane w dużo większy kwadrat. Gracz nie mógł podejść do ściany w zasięgu wspinaczki (1,7 m) prawie nigdzie poza płaskimi fasadami. Naprawa: `tycho_city.gd:_add_precise_footprint_collider` mierzy realny ślad budynku siatką promieni z góry na jego własnym trimeshu i dopasowuje dwa pudełka dla L-kształtu (lustrzane dla `descriptor.mirror`) albo `ConvexPolygonShape3D`/`CylinderShape3D` z zmierzonych punktów obwodu dla pozostałych. (2) Animacja Stand_Up2 potrafiła zapętlić się w nieskończoność stojąc na dachu, który realnie ją utrzymywał — `_physics_stand_up()` sprawdzał `_roof_can_stand()` co klatkę na ŻYWEJ, aktualnie odtwarzanej pozycji kości stóp, więc pojedyncza klatka, w której noga w trakcie wstawania z czworaków wychyliła się poza krawędź dachu, cofała ją do pełzania i natychmiast uruchamiała Stand_Up2 od nowa. Naprawa: krótki okres łaski (`STAND_UP_SUPPORT_GRACE`, 0,25 s) zanim animacja faktycznie się przerwie — ten sam wzorzec debounce co `CLIMB_WALL_MISS_GRACE` przy oknach/drzwiach podczas wspinaczki.

Wniosek ogólny: zielone testy jednostkowe (`godot/tests/*.gd`, syntetyczne pudełka) nie są dowodem, że mechanika działa w realnej grze — dopiero rzeczywisty playtest w zbudowanej grze wyłapał oba te błędy, mimo że test facade-depth (patrz wyżej) już wcześniej przeszedł.
